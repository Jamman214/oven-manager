...
class PresetManager():
    ...
    @staticmethod
    def get_limits(time):
        preset_id = PresetManager.get_active()['id']
        ...
        week_preset = DatabaseManager.query_db(
            extract_week_preset,
            args={'preset_id': preset_id, 'time': time},
            one=True
        )

        if week_preset is not None:
            day_id_keys = ['monday_preset_id', 'tuesday_preset_id', 'wednesday_preset_id', 'thursday_preset_id', 'friday_preset_id', 'saturday_preset_id', 'sunday_preset_id']
            day = datetime.fromtimestamp(time, tz=DatabaseManager.tz).weekday()
        ...
        day_presets = DatabaseManager.query_db(
            extract_day_presets,
            args={'preset_id': preset_id, 'time': time}
        )

        if len(day_presets) > 0:
            cur_time = datetime.fromtimestamp(time, tz=DatabaseManager.tz)
            day_start = day.replace(hour=0, minute=0, second=0, microsecond=0)
            for chunk in day_presets:
                if (
                    day_start + timedelta(seconds=chunk['start']) <= cur_time
                    and day_start + timedelta(seconds=chunk['end']) > cur_time
                ):
        ...
        atomic_preset = DatabaseManager.query_db(
            extract_atomic_preset,
            args={'preset_id': preset_id, 'time': time},
            one=True
        )
